...
| The dev server does not restart itself after a merge | when it next bites | Twice now a reported bug has turned out to be a stale server process: it keeps serving the build it started with, so newly added endpoints 404. `curl -m 5 localhost:5080/api/health` proves it is up, not that it is current |
| Health of the dev loop: nothing tells you the backend is down | done, 2026-08-05 | Under WSL2 a connection to a port nothing listens on is accepted and never answered, so the browser waited forever and the app sat on "Loading…". The client now aborts after 10s and says the server is unreachable, distinct from "not signed in" |
